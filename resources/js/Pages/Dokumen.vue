                        ...
                        <ul class="space-y-1 pl-2">
                            <li v-for="item in category.items" :key="item.id">
                                <Link
                                    ...
                                    preserve-scroll
                                    :class="[
                                        'flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-all duration-300 w-full relative overflow-hidden shadow-sm',
        ...
        <main class="flex-1 p-6 md:p-12 h-screen overflow-y-auto scroll-smooth relative">
            ...
            <div class="animate-fade-in max-w-4xl mx-auto pb-20 relative z-10">
                ...
                <div v-if="activeDocument">
                    <h1 class="font-headline text-4xl md:text-5xl text-pkl-dark-blue mb-6 leading-tight uppercase">
                        {{ activeDocument.name }}
                    </h1>
                    
                    <div class="bg-white rounded-[2rem] shadow-xl border border-white/50 overflow-hidden mb-10 p-8">
                        <div class="h-[60vh] bg-gray-50 flex items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl mb-8">
                            <p class="text-gray-400 font-sub text-lg">
                                [PDF Viewer / Tampilan Dokumen Disini]
                            </p>
                        </div>
                        <div class="flex justify-end">
                            <a :href="activeDocument.file_url" target="_blank" class="inline-flex items-center px-8 py-3 bg-pkl-base-orange text-white font-bold rounded-full shadow-lg hover:bg-orange-600 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                ...
                <div v-else class="h-full flex flex-col items-center justify-center text-center opacity-80 mt-20">
                     <h2 class="font-headline text-4xl text-pkl-compliment-blue mb-3">Arsip Data & Laporan</h2>
                     <p class="font-sub text-gray-500 max-w-md">Silakan pilih dokumen dari navigasi di sebelah kiri untuk melihat pratinjau dan mengunduh file.</p>
                </div>
