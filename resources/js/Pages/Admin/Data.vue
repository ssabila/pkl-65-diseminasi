...
const histogramBinSize = ref(10) // User input range/bin size
...
// Update histogram preview when auto-calculate is toggled
...
const parseHistogramFile = async (file) => {
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                // Parse data: support comma, semicolon, space, tab, and newline separators
                const values = text
                    .split(/[\s,;\t\n\r]+/)
                    .map(v => v.trim())
                    .filter(v => v !== '')
...
const handleHistogramFileChange = async (event) => {
    ...
    try {
        const values = await parseHistogramFile(file)
        if (values.length === 0) {
        ...
        // Store parsed data as text for consistency with existing code
        ...
        notificationTitle.value = 'Parsing Gagal!'
        notificationMessage.value = 'Gagal membaca file. Pastikan file berisi data numerik.'
        showErrorNotif.value = true
    }
}
...
const initializeEditingState = async () => {
    ...
    if (isBarOrPie.value) {
        ...
        if (rawData && Array.isArray(rawData) && rawData.length > 0) {
            ...
            histogramBinSize.value = editing.chart_data?.binSize || 10
            histogramAutoCalculate.value = editing.chart_data?.autoCalculate || false
        }
        chartCategories.value = []
    ...
    Kelola Diseminasi
        ...
        <main class="max-w-6xl mx-auto p-4 sm:p-4">
            ...
            <div class="bg-[var(--color-surface)] rounded-lg shadow-md border border-[var(--color-border)] mb-6 overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    <div v-if="isHistogram" class="border-t pt-6">
                        ...
                        <div class="mb-6">
                            <label class="block text-sm font-medium mb-3">Metode Input Data:</label>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-blue-500 transition-colors">
                                    <div class="text-center">
                                        ...
                                        <p class="text-xs text-gray-500 mt-2">TXT, CSV, atau Excel</p>
                                        
                                        <!-- File info if uploaded -->
                                        <div v-if="histogramFile" class="mt-3 p-2 bg-green-50 rounded border border-green-200">
                                            <div class="flex items-center justify-between text-sm">
                                                <span class="text-green-700 truncate">📄 {{ histogramFile.name }}</span>
                                                <button @click="clearHistogramFile" type="button" class="text-red-500 hover:text-red-700 ml-2">
                                ...
                                <div class="border-2 border-gray-300 rounded-lg p-6">
                                    <div class="text-center mb-3">
                                        ...
                                        <p class="text-sm font-medium text-gray-700">Input Manual</p>
                                        <p class="text-xs text-gray-500 mt-1">Ketik langsung di textarea</p>
                                    </div>
                                    <textarea
                                        v-model="histogramRawData"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
                                        rows="4"
                                        placeholder="12, 15, 18, 22, 25, 28, 30..."
                                    ></textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-6 border rounded-lg p-4 bg-gray-50">
                            ...
                            <div class="mb-4 flex items-center gap-3">
                                <label class="inline-flex items-center cursor-pointer">
                                    ...
                                    <span class="ms-3 text-sm font-medium text-gray-700">Hitung Otomatis (Optimal)</span>
                                </label>
                            </div>
                            ...
                            <div v-else class="p-3 bg-green-50 rounded-lg border border-green-200">
                                <p class="text-sm text-green-700">
                                    ✓ Sistem akan menghitung ukuran bin optimal berdasarkan distribusi data menggunakan metode Freedman-Diaconis
                                </p>
                            </div>
                        </div>
                    ...
                    <div v-if="isBoxPlot" class="border-t pt-6">
                        ...
                        <div class="space-y-4">
                            <div v-for="(group, index) in boxPlotData" :key="index" 
                                ...
                                <div v-if="group.min && group.q1 && group.median && group.q3 && group.max" class="mt-2">
                                    <div v-if="parseFloat(group.min) <= parseFloat(group.q1) && 
                                            parseFloat(group.q1) <= parseFloat(group.median) && 
                                            parseFloat(group.median) <= parseFloat(group.q3) && 
                                            parseFloat(group.q3) <= parseFloat(group.max)"
                                    ...
                                    <div v-else class="text-xs text-red-600 flex items-center gap-1">
                                        ...
                                        Urutan nilai tidak valid (harus: Min ≤ Q1 ≤ Median ≤ Q3 ≤ Max)
                                    </div>
                                </div>
                            </div>
            ...
            >
                ...
                <!-- Box Plot Preview -->
                <!-- Box Plot Preview -->
                <div v-if="isBoxPlot" class="mb-6 bg-white rounded-lg p-4">
                    <ApexBoxPlotChart 
